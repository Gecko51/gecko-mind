...
const agents = [
  {
    ...
    image: linkIcon,
    description:
      "De l'onboarding à la relance, transformez votre charge mentale en flux de travail automatisé et économisez 10 heures par semaine",
    color: "text-blue-400",
    tasks: [
  ...
  return (
    <section id="ai-agents" className="py-20 bg-black">
      <div className="container mx-auto px-6">
        ...
        <div
          ref={cardsRef}
          className={`grid lg:grid-cols-3 gap-8 transition-all duration-1000 ${cardsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}
        >
          {agents.map((agent, index) => {
            ...
            return (
              <Card
                key={index}
                className={`relative p-8 bg-gradient-card border-border/50 backdrop-blur-sm group hover:shadow-glow-primary/20 transition-all duration-300 hover:-translate-y-2 ${cardsVisible ? "animate-fade-in" : ""}`}
                style={{
