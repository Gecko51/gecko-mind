...
const agents = [
  {
    ...
    tasks: [
      "Onboarding leads/client automatisé",
      "CRM sur-mesure",
      "Synchronisation de l'agenda",
      "Recherche ciblée de leads",
      "Outreach automatisé",
      "Setting & lead nurturing",
  ...
  return (
    <section id="ai-agents" className="py-20 bg-black">
      <div className="container mx-auto px-6">
        ...
        <div
          ref={cardsRef}
          className={`flex justify-center transition-all duration-1000 ${cardsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}
        >
          {agents.map((agent, index) => {
            ...
            return (
              <Card
                key={index}
                className={`relative p-8 bg-gradient-card border-border/50 backdrop-blur-sm group hover:shadow-glow-primary/20 transition-all duration-300 hover:-translate-y-2 max-w-md ${cardsVisible ? "animate-fade-in" : ""}`}
                style={{
                ...
                <div className="flex flex-col items-center text-center">
                  {agent.image ? (
                    <div className="flex flex-col items-center mb-6">
                      <img src={agent.image} alt={agent.name} className="w-12 h-auto object-contain" />
                  ...
                  <p className="text-lg bg-gradient-to-r from-secondary to-primary bg-clip-text text-transparent leading-relaxed mb-6">
                    {agent.description}
                  </p>
                  ...
                  {agent.tasks.length > 0 && (
                    <div className="space-y-3 mt-4 w-full">
                      {agent.tasks.map((task, taskIndex) => (
                        <div key={taskIndex} className="flex items-center gap-3 text-left">
        ...
        <div className="text-center mt-12">
          <div className="mb-16 text-left max-w-3xl mx-auto">
            <h3 className="text-2xl font-bold mb-4 pt-8 bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
          ...
          <div className="mb-16 text-left max-w-3xl mx-auto">
            ...
            <ul className="space-y-4 text-white/90">
              <li className="flex items-start gap-3">
                <CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                <span>
                  Une plateforme privée de vidéos pédagogiques avec tous les tutoriels pour maîtriser chaque outil du
                  système et comprendre la stratégie derrière chaque clic.
                </span>
              </li>
              <li className="flex items-start gap-3">
                <CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
          ...
          <p className="text-2xl font-bold text-white pt-8">
            Tarif du système complet
            <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent"> 2795 €</span>
          </p>
